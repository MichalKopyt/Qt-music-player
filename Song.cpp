...
void Song::on_media_status_changed(QMediaPlayer::MediaStatus status)
{
    if (metadataLoader->error() != QMediaPlayer::NoError) {
        qDebug() << "Error while opening file: " << fileName() << ". Please check this file or codecs on your system." ;
        delete this;
        return;
    }
    if (status == QMediaPlayer::LoadedMedia){
        if (metadataLoader->error() != QMediaPlayer::NoError || !(metadataLoader->isAudioAvailable())) {
            qDebug() << "Error while opening file: " << fileName() << ". Please check this file or codecs on your system." ;
            delete this;
            return;
        } else if (metadataLoader->isMetaDataAvailable()) {
